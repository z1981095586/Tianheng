...
<script>
  ...
  export default {
    ...
    methods: {
           scan() {
   nativeMethod.scan1(null,null,this.macRelation.workshop_id,this.macRelation.mac_type_id,this.selectInfo.company_id,this.days_to_expire,null)
        // this.$router.push({
        //   path: '/choose',
        //   name: 'choose',
        //   params: {
  ...
  let url = 'http://47.110.242.174:10086/report/getSimpleReport';
  let headers = {
...
that.mac_type.push( response.data.data[i])
      ...
      change(e) { //workshop下拉选择框值改变重新获取数据
        this.datalist = []
          this.page_num=1
 ...
 this.getMachine_id()
      ...
      getWorkShopList() { //获取车间列表数据
        let url = " http://47.110.242.174:8211/workShop/getWorkShopList"
        let that = this
        axios.post(url, {
          page: 1,
          pageSize: 10000
