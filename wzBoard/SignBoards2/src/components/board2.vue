<template>
  <div class="content">
    <header style="height: 3rem">
      <div class="title">万舟织造二部——万舟织造车间电视看板</div>
          ...
          <div class="board_con_right_top">
            ...
            <div class="board_con_right_top_con">      <div class="board_con_right_top_con2_left"><span style="font-size:2rem;width:4rem">断头排名</span></div>
                ...
                <div class="board_con_right_top_con2_right_con" v-for="(item,index) in dtpmList" :key="index"><span
                    style="font-size:2.5rem;width:50%;text-align:left;margin-left:10%">#{{item.machineId}}</span><span
                    style="font-size:2.5rem;color:rgb(213,119,80);width:40%;text-align:left">{{item.weftCmpx}}%</span></div>
  ...
  let that=this
      ...
      //  that.speed=response.data.data.selects[0].rpm
          ...
          .then(() => {
                ...
                axios({
            url: host1+':7070/app/getMachineRepairBoard', //设备维修
            method: 'post',
            headers: {
              'Content-Type': 'application/json',
              'companyId': that.companyId
            },
            data:{"workshopId":that.workshopId,"groupId":null}
            ...
            let list=[]
             for(let i=0;i<response.data.data.data.length;i++){
               if(response.data.data.data[i].status=="已完成"){
                 response.data.data.data[i].color="color:rgb(20, 234, 26)"
               }else{
