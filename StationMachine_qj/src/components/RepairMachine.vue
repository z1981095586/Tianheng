<template>
  <div class="operationPane">
      ...
      <div class="operationPane_con_machineList">
        ...
        >
          <el-checkbox-button
            ...
            ><span style="margin: 0.5rem">{{ item.repair_typename }}</span
            ><span>上报时间：{{ item.submit_date }}</span></el-checkbox-button
          >
        </el-checkbox-group>
      </div>
    ...
    >
      <div class="operationPane_con_machineList2">
        <el-checkbox-group
          :max="1"
          @change="checkedType"
          v-model="checkType"
      ...
      <div class="search" style="left: 3rem; width: 95%; top: 18px">
        ...
        <span style="color: red">维修机台：{{ this.checkedMachineNum }}</span>
      </div>
    ...
    >
      <div class="Maintance_machine">
        ...
        }}<span style="margin-left: 2rem">上报时间:{{ time }}</span>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      company_id: 10000015,
      isWaitMaintance: 2, //是否待保养，0带保养，1保养预期，2待维修
      reasonlist: [
        {
          label: "经停",
          value: "经停",
  ...
  methods: {
    ...
    getAllmaterial() {
      ...
      axios
        ...
        .then(function (res) {
          ...
          that.total_num3 = res.data.totalDataNum; //设置数据总条数
          ...
          for (let i = 0; i < res.data.data.productModel.length; i++) {
            ...
            that.allMaterialList.push(res.data.data.productModel[i]);
          }
        });
    },
    lastPage2() {
      //所有物料上一页
      if (this.page_num3 > 1) {
        this.page_num3 = this.page_num3 - 1;
        this.getAllmaterial();
      } else {
        this.$message({
          message: "没有上一页了",
    ...
    nextPage2() {
      //所有物料下一页
      if (this.total_num3 <= this.page_num3 * this.page_size3) {
        this.$message({
          message: "没有下一页了",
          type: "warning",
        });
  ...
  watch: {
    ...
    checkedMachineNum(val) {
      this.machineListCon.forEach((element) => {
        if (element.machine_id == val) {
          this.time = element.submit_date;
        }
      });
    },
    //监听页面显示执行相应的获取数据函数
...
.operationPane_con_machineList2 {
  width: 93%;
  height: 60%;
  margin-top: 9%;
  display: -webkit-box;
...
.operationPane_con_machineList2 /deep/ .el-checkbox-button__inner {
  font-size: 2rem;
  padding: 20px 27px;
  margin: 1rem 0.5rem;
}
