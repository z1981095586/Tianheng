<template>
  <div class="content">
    <div class="all-page">
      ...
      <div
        style="width: 99%; display: flex; align-items: center; justify-content: center"
      >
        <div
          style="
            width: 77%;
          ...
          "
        >
          <span class="spans">详情</span
          ><span class="spans">姓名：{{ jsonData.staff_name }}</span
          ><span class="spans">工号：{{ jsonData.staff_id }}</span
          ><span class="spans">厂区：{{ jsonData.CompanyID }}</span
          ><span class="spans">车间：{{ jsonData.WorkShopID }}</span>
          <div
            style="
              display: flex;
              flex-direction: column;
          ...
          >
            <span>机台：</span
            ><el-input
              style="font-size: 1.2rem; width: 90%; margin-left: 0.6rem"
              type="textarea"
              disabled
              autosize
...
export default {
  ...
  methods: {
    ...
    getData() {
      //先查lm_shift,再查人员信息s_staff,人员信息数据用s_staff覆盖
      //获取数据
      ...
      axios({
        ...
        if (that.value != "机台号") {
          ...
          axios({
            ...
            data: {
              tableName: "s_staff",
              selectFields: ["workshop_id", "company_id", "staff_name", "staff_code"],
              query: query,
            },
            headers: {
              companyId: that.selectInfo.company_id,
            ...
            if (res2.data.data.length > 0) {
              that.jsonData.staff_name = that.isNullData(
                that.jsonData.staff_name,
                res2.data.data[0].staff_name
              );
              // that.jsonData.WorkShopID = that.isNullData(
              //   that.jsonData.WorkShopID,
              //   res2.data.data[0].workshop_id
              // );
              that.jsonData.staff_id = that.isNullData(
                that.jsonData.staff_id,
                res2.data.data[0].staff_code
              );
              that.jsonData.CompanyID = that.isNullData(
                that.jsonData.CompanyID,
                res2.data.data[0].company_id
              );
            } else {
              that.$message({
                message: "没有查询到人员信息！",
