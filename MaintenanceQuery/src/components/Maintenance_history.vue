<template>
  <div class="content" style="padding-top: 10px">
    ...
    <div class="all-page">
      <div class="query">
        <span style="margin-left: 10px; font-size: 0.5em; margin-right: 10px"
          >开始时间</span
        >
        <el-date-picker
          v-model="start_time"
        ...
        <el-input
          id="myInput2"
          v-model="macRelation.machine_id"
          size="small"
          style="width: 20%"
        ></el-input>
      </div>
      ...
      <scroller height="100%" :onRefresh="refresh" :onInfinite="inf" ref="my_scroller">
        <div class="scroller">
          <div
            class="card"
            @click="toDetail(item.id, item.maintain_type_name, item.machine_id)"
            v-for="(item, index) in dataList"
            :key="index"
          >
            <div class="card-con">
              <span>保养项目：{{ item.maintain_type_name }}</span>
            </div>
            <div class="card-con2" style="margin-top: 10px">
              <span>保养设备：{{ item.machine_id }}</span>
            </div>
            <div class="card-con2">
              <span>保养完成时间：{{ item.maintain_time }}</span>
            </div>
            <div class="card-con2">
              <span>保养人：{{ item.operator }}</span>
            </div>
