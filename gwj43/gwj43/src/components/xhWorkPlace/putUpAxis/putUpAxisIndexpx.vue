            ...
            <div class="left-bottomDiv" id="leftDiv"  :style="" style="overflow-y: hidden;height: 90%" @mousedown="mousedown" @touchstart="mousedown" @mousemove="move" @mouseup="end" @touchmove.prevent="move"  @touchend="end" >
              <div class="standard_border" :style="{height:scrollerHeightBottomDiv,backgroundColor:getLabelColor(item.status)}" v-for="item in all_order_list" :label="item.barCode" :key="item.id" style="width: 90%;display: inline-block;margin-left: 5%;margin-top: 5px;border-width:2px;padding: 10px" >
                <table cellspacing='20px' cellpadding='20px' border="0" width="100%">
                  ...
                  <tr>
                    <td colspan="2">
                      <p class="standard_font" style="font-size: 1.5vw;font-weight: bolder">编号{{plan_code}}</p>
                    </td>
                  </tr>
                  <tr>
                    <td colspan="2">
                      <p class="standard_font" style="font-size: 1.5vw;font-weight: bolder">预排产机台号：&nbsp{{item.machineIdPlan || "无"}}</p>
                    </td>
                  </tr>
                  <tr>
                    <td colspan="2">
                      <p class="standard_font" style="font-size: 1.5vw;font-weight: bolder">排产机台号：&nbsp{{item.machineId || machineIdSelected}}</p>
                    </td>
                  </tr>
                  <tr>
                    <td colspan="2">
                      <p class="standard_font" style="font-size: 1.5vw;font-weight: bolder">织轴号：&nbsp{{item.axisNo}}</p>
                    </td>
                  </tr>
                  <tr>
                    <td colspan="2">
                  ...
                  <tr>
                    <td>
                      <el-button type="success" style="width: 7.5vw;height: 6vw;margin-bottom: 5px;line-height:1.4;letter-spacing:4px" size="medium"  @click="putUpAxis">
        ...
        <div :style="{height: scrollerHeightBottom,width:scrollerWidthLeft}"  style="display: inline-block;margin-left: 10px">
          <el-card style="width: 100%;" :style="{height: scrollerHeightBottom,width:scrollerWidthRight}" shadow="hover" >
            ...
            <div class="left-bottomDiv" :style="{height: scrollerHeightBottomDiv}" id="rightDiv"  @mousedown="mousedown1" @touchstart="mousedown1" @mousemove="move1" @mouseup="end1" @touchmove.prevent="move1"  @touchend="end1" >
              <table class="radioTable"  style="width: 90%" cellspacing="20px" v-show="radio === 'radio1'&&all_order_list[0]">
                <tr>
                  <td style="text-align: center;width: 33%" @click="">
                    <div style="height:15vh" @click="showAddClothDialog = true">
                  ...
                  <td id="0" style="text-align: left;width: 33%"  v-if="order_detail_list.length>0" @click="setClothId(0)">
                    <div style="height:15vh;margin-left: 2%;width: 96%">
                      <div id="0title" style="height:5vh;font-size: 1.6vw;border-bottom: 1px solid lightgray;font-weight: bolder">{{order_detail_list[0].id}}</div>
                  ...
                  <td id="1" style="text-align: left;width: 33%" v-if="order_detail_list.length>1" @click="setClothId(1)">
                    <div style="height:15vh;margin-left: 2%;width: 96%">
                      <div id="1title" style="height:5vh;font-size: 1.6vw;border-bottom: 1px solid lightgray;font-weight: bolder">{{order_detail_list[1].id}}</div>
                ...
                <tr v-for="indexTr in order_detail_list_length">
                  <td :id="indexTd+indexTr*3-2" v-for="indexTd in 3" v-if="(indexTd+indexTr*3-1)<=order_detail_list.length"  style="text-align: left;width: 33%" @click="setClothId(indexTd+indexTr*3-2)">
                    <div style="height:15vh;margin-left: 2%;width: 96%">
                      <div :id="indexTd+indexTr*3-2 + 'title'" style="height:5vh;font-size: 1.6vw;border-bottom: 1px solid lightgray;font-weight: bolder">{{order_detail_list[indexTd+indexTr*3-2].id}}</div>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      addCloth(){
        let id = this.order_detail_list.length+1;
        console.log(this.order_detail_list);
        let code = this.barCode.substr(0,this.barCode.length-3);
        let message = {};
        let data = {};
        data= [];
        let order_detail_list = [];
        ...
        for (let i = 0; i < this.addNumber; i++) {
          message = {};
          message.barId = code + "0" + id;
          message.id = "FB" + message.barId.substring(2);
          message.orderPlanId = null;
          message.productName = this.all_order_list[0].productName;
          message.barCode = code;
          message.machineId = this.machineIdSelected;
          message.yieldMeterSemi = "110M";
          message.yieldMeter = message.yieldMeterSemi;
          message.printCount = 1;
          data.push(message);
          order_detail_list.push({
            currentStaffId: this.staff_id,
            currentRealYield:"110M",
            id:code + "0" + id,
            axisNo: this.all_order_list[0].axisNo,
            print_count:1
          });
      ...
      endCloth(printNum){
        ...
        data= [];
        let order_detail_list = [];
        this.newCodeNum = printNum;//用于标志新加布票
      ...
      putUpAxis(){
        ...
        // data.printCode = "PJ" + this.plan_code.substring(2);
